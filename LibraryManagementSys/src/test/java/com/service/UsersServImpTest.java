...
class UsersServImpTest {
	...
	void testRegisterUserDetails() {
		...
		ur.setUserid(1);
		Mockito.when(repo.save(ur)).thenReturn(ur);
	...
	void testUpdateUserDetails() {
		...
		Mockito.when(repo.findById(1)).thenReturn(u1);
		Mockito.when(repo.save(ur)).thenReturn(ur);
	...
	void testDeleteUserDetails() {
		...
		Optional<Users> u1 =  Optional.of(ur);
		Mockito.when(repo.findById(1)).thenReturn(u1);
		Mockito.when(repo.existsById(ur.getUserid())).thenReturn(false);
		assertFalse(repo.existsById(ur.getUserid()));
	...
	void testGetEmail() {
		 Users ur = new Users();
			...
			ur.setSubscriptionstatus("valid");
			ur.setUserid(1);
			Mockito.when(repo.findByEmail("jayasri02@gmail.com")).thenReturn(ur);
	...
	void testGetMobileno() {
		  ...
		  Users ur = new Users();
			...
			ur.setSubscriptionstatus("valid");
			ur.setUserid(1);
			Mockito.when(repo.findByMobileno("9876543220")).thenReturn(ur);
